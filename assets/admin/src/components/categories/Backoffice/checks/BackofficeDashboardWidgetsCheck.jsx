	...
	return (
		<ReportCard result={result} actions={null}>
			<div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm">
				...
				{brandaActive && !moduleActive ? <button type="button" disabled={isFixBusy(result)} className="mt-3 inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-900 shadow-sm transition hover:-translate-y-0.5 hover:border-[rgb(var(--au-primary-rgb)/0.3)] disabled:cursor-not-allowed disabled:opacity-60" onClick={() => onFix(result)}>{isFixBusy(result) ? <span className="inline-block h-4 w-4 shrink-0 animate-spin rounded-full border-2 border-slate-300 border-t-slate-800" aria-hidden /> : null}Corriger (activer le module)</button> : null}
				{brandaActive && moduleActive && availableCount > 0 && !allHidden ? <button type="button" disabled={isFixBusy(result)} className="mt-3 inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-900 shadow-sm transition hover:-translate-y-0.5 hover:border-[rgb(var(--au-primary-rgb)/0.3)] disabled:cursor-not-allowed disabled:opacity-60" onClick={() => onFix(result)}>{isFixBusy(result) ? <span className="inline-block h-4 w-4 shrink-0 animate-spin rounded-full border-2 border-slate-300 border-t-slate-800" aria-hidden /> : null}Masquer tous les widgets</button> : null}
