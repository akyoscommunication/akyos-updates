function UpToDateBadge({ upToDate }) {
	if (upToDate === false) {
		return (
			<span className="inline-flex rounded-full bg-amber-500/15 px-2 py-0.5 text-xs font-semibold text-amber-900">Non</span>
		);
	}
	if (upToDate === true) {
		return (
			...
			<table className="w-full border-collapse text-left text-[13px]">
				<thead>
					<tr>
						...
						<th className="border-b border-slate-200 px-2 py-2">À jour</th>
					...
					{plugins.map((plugin, index) => (
						<tr key={`${plugin.slug}-${index}`}>
							...
							<td className="border-b border-slate-100 px-2 py-2 align-middle">
								<UpToDateBadge upToDate={plugin.upToDate} />
							</td>
							<td className="border-b border-slate-100 px-2 py-2 align-middle">
